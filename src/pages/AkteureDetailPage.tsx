...
import { formEnhancements } from '@/config/form-enhancements/Akteure';
import { evalComputed } from '@/config/form-enhancements/types';

export default function AkteureDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Akteure | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getAkteure();
  ...
  async function handleUpdate(fields: Akteure['fields']) {
    if (!record) return;
    await LivingAppsService.updateAkteureEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteAkteureEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/akteure');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        ...
        action={
          <Button variant="ghost" onClick={() => navigate('/akteure')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/akteure')}
      onEdit={() => setEditing(true)}
      backLabel="Zurück"
      editLabel="Bearbeiten"
    >
      <RecordHeader title={record.fields.vorname ?? 'Akteure'} />
