...
import { formEnhancements } from '@/config/form-enhancements/ExterneObjekte';
import { evalComputed } from '@/config/form-enhancements/types';

export default function ExterneObjekteDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<ExterneObjekte | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getExterneObjekte();
  ...
  async function handleUpdate(fields: ExterneObjekte['fields']) {
    if (!record) return;
    await LivingAppsService.updateExterneObjekteEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteExterneObjekteEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/externe-objekte');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        ...
        action={
          <Button variant="ghost" onClick={() => navigate('/externe-objekte')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/externe-objekte')}
      onEdit={() => setEditing(true)}
      backLabel="Zurück"
      editLabel="Bearbeiten"
    >
      <RecordHeader title={record.fields.systemname ?? 'Externe Objekte'} />
