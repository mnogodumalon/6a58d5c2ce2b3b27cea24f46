...
import { formEnhancements } from '@/config/form-enhancements/Knotenbeziehungen';
import { evalComputed } from '@/config/form-enhancements/types';

export default function KnotenbeziehungenDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Knotenbeziehungen | null>(null);
  ...
  async function handleUpdate(fields: Knotenbeziehungen['fields']) {
    if (!record) return;
    await LivingAppsService.updateKnotenbeziehungenEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteKnotenbeziehungenEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/knotenbeziehungen');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        ...
        action={
          <Button variant="ghost" onClick={() => navigate('/knotenbeziehungen')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/knotenbeziehungen')}
      onEdit={() => setEditing(true)}
      backLabel="Zurück"
      editLabel="Bearbeiten"
    >
      <RecordHeader title={'Knotenbeziehungen'} />
      ...
      <RecordSection title="Details" cols={2}>
        <RecordField label="Quellknoten" value={getBedeutungsknotenDisplayName(record.fields.quellknoten)} format="text" />
        <RecordField label="Zielknoten" value={getBedeutungsknotenDisplayName(record.fields.zielknoten)} format="text" />
        ...
        <RecordField label="Ersteller" value={getAkteureDisplayName(record.fields.beziehung_ersteller)} format="text" />
