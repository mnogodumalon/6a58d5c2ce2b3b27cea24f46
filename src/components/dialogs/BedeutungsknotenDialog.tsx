...
import type { Bedeutungsknoten, Akteure, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Bedeutungsknoten';
...
interface BedeutungsknotenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Bedeutungsknoten['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Bedeutungsknoten['fields'], 'knotentyp' | 'status'> & {
    knotentyp?: LookupValue | string;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  knotentyp: LOOKUP_OPTIONS['bedeutungsknoten']?.['knotentyp'] ?? [],
  status: LOOKUP_OPTIONS['bedeutungsknoten']?.['status'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  ersteller: APP_IDS.AKTEURE,
};
...
export function BedeutungsknotenDialog({ open, onClose, onSubmit, defaultValues, recordId, akteureList, enablePhotoScan = true, enablePhotoLocation = true }: BedeutungsknotenDialogProps) {
  const [fields, setFields] = useState<Partial<Bedeutungsknoten['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['titel', 'knotentyp', 'status'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Bedeutungsknoten['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'bedeutungsknoten');
      await onSubmit(clean as Bedeutungsknoten['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "titel": string | null, // Titel\n  "beschreibung": string | null, // Beschreibung\n  "knotentyp": LookupValue | null, // Knotentyp (select one key: "intent" | "goal" | "decision" | "effect" | "obstacle" | "evidence" | "principle" | "hypothesis" | "question") mapping: intent=Absicht, goal=Ziel, decision=Entscheidung, effect=Effekt, obstacle=Hindernis, evidence=Evidenz, principle=Prinzip, hypothesis=Hypothese, question=Frage\n  "status": LookupValue | null, // Status (select one key: "draft" | "active" | "completed" | "obsolete") mapping: draft=Entwurf, active=Aktiv, completed=Abgeschlossen, obsolete=Veraltet\n  "ersteller": string | null, // Display name from Akteure (see <available-records>)\n  "erstellt_am": string | null, // YYYY-MM-DDTHH:MM\n  "aktualisiert_am": string | null, // YYYY-MM-DDTHH:MM\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["ersteller"]);
        ...
        if (erstellerName) {
          const erstellerMatch = akteureList.find(r => matchName(erstellerName!, [[r.fields.vorname ?? '', r.fields.nachname ?? ''].filter(Boolean).join(' ')]));
          if (erstellerMatch) merged['ersteller'] = createRecordUrl(APP_IDS.AKTEURE, erstellerMatch.record_id);
        }
        return merged as Partial<Bedeutungsknoten['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Bedeutungsknoten bearbeiten' : 'Bedeutungsknoten hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        ...
        <Textarea
          id="beschreibung"
          placeholder="Was bedeutet dieser Knoten? Kontext, Hintergrund, Implikationen..."
    ...
    'knotentyp': (
      <div key="knotentyp" className="space-y-1.5">
        <Label htmlFor="knotentyp">Knotentyp <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Select
          value={lookupKey(fields.knotentyp) ?? ''}
          onValueChange={v => setFields(f => ({ ...f, knotentyp: v === 'none' ? undefined : v as any }))}
        >
          <SelectTrigger id="knotentyp" className="max-sm:h-11"><SelectValue placeholder="Typ des Knotens wählen" /></SelectTrigger>
          <SelectContent>
    ...
    'ersteller': (
      <div key="ersteller" className="space-y-1.5">
        <Label htmlFor="ersteller">Ersteller</Label>
        <Combobox
          id="ersteller"
          ...
          onChange={id => setFields(f => ({ ...f, ersteller: id ? createRecordUrl(APP_IDS.AKTEURE, id) : undefined }))}
          searchPlaceholder="Suchen…"
          emptyText="Kein Treffer"
          onCreateNew={(q) => openCreateAkteure("ersteller", q)}
    ...
    'aktualisiert_am': (
      <div key="aktualisiert_am" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"titel": "Titel", "beschreibung": "Beschreibung", "knotentyp": "Knotentyp", "status": "Status", "ersteller": "Ersteller", "erstellt_am": "Erstellt am", "aktualisiert_am": "Aktualisiert am"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"ersteller": {"vorname": "Vorname", "nachname": "Nachname", "name_organisation": "Name (Organisation / Team / System)", "akteurstyp": "Akteurstyp", "externer_verweis": "Externer Verweis"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.BEDEUTUNGSKNOTEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
